...
export default function ClaudeRunner3D() {
  ...
  useEffect(() => {
    ...
    window.claude.onUsageUpdate((data) => {
      prevPctRef.current = pctRef.current;
      ...
      if (data.weekly?.resetAt) {
        setWeeklyResetMs(Math.max(0, new Date(data.weekly.resetAt) - Date.now()));
      }
    });
  }, []);
  ...
  useEffect(() => {
    const iv = setInterval(() => {
      setResetMs(ms => Math.max(0, ms - 1000));
      setWeeklyResetMs(ms => Math.max(0, ms - 1000));
    }, 1000);
      ...
      <div style={{ background:'rgba(24,24,30,0.97)', borderRadius:14, border:'1px solid rgba(255,255,255,0.07)', boxShadow:'0 8px 30px rgba(0,0,0,0.5)', overflow:'hidden' }}>
        <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between', padding:'7px 12px 5px', borderBottom:'1px solid rgba(255,255,255,0.05)', WebkitAppRegion:'drag' }}>
          <div style={{ display:'flex', alignItems:'center', gap:5, WebkitAppRegion:'no-drag' }}>
            ...
            <div onClick={()=>window.claude?.minimize()} style={{ width:12, height:12, borderRadius:'50%', background:'#FEBC2E', cursor:'pointer' }} />
        ...
        <div style={{ padding:'0 12px 6px' }}>
          ...
          <div style={{ marginBottom:5 }}>
            ...
            <div style={{ fontSize:11, color:'rgba(255,255,255,0.5)', marginTop:3, fontWeight:500 }}>{fmtResetLabel(weeklyResetMs)}</div>
          </div>
